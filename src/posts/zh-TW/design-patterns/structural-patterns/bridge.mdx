export const metadata = {
  ...
  date: '2026-06-30',
  excerpt: '介紹結構型設計模式中的橋接模式：如何將抽象與實作分離，避免繼承層級爆炸，讓兩邊可以各自獨立演進。',
  tags: ['軟體設計', '設計模式', 'OOP'],
};
...
// Abstraction：遙控器持有一個指向 Device 的引用 (這就是橋)
class RemoteControl {
...
// RefinedAbstraction：擴充功能，完全不動 Device 層
...
一旦發現某個類別可能往兩個彼此獨立的方向成長，就該考慮把其中一個維度抽出來、做成獨立的介面，這就是橋。如此一來，兩邊都能完整擴充，而不必互相牽動。
