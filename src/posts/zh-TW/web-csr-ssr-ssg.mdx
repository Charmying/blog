...
```mermaid
flowchart LR
    req["瀏覽器請求"] --> html["空白 HTML + JS"] --> js["執行 JS"] --> api["請求 API"] --> render["渲染畫面"]
```
...
```mermaid
flowchart LR
    req["瀏覽器請求"] --> fetch["伺服器取得資料"] --> gen["產生 HTML"] --> full["完整 HTML"] --> show["顯示內容"]
```
...
flowchart TD
    subgraph build["建置時"]
        fetch["取得資料"] --> gen["產生所有 HTML 檔案"]
    end
    subgraph runtime["使用者請求"]
        req["使用者請求"] --> cdn["CDN 直接回傳靜態 HTML"]
    end
```
...
CSR / SSR / SSG 是 Runtime 階段的渲染策略。無論採用哪種策略，瀏覽器收到 HTML 後都要走過同樣的渲染流程，參考 [瀏覽器渲染流程](/articles/web-browser-rendering)；想了解完整執行週期，參考 [從原始碼到畫面](/articles/web-sourcecode-to-screen)。
