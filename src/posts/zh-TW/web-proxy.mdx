...
```mermaid
flowchart LR
    client["客戶端"] --> proxy["Forward Proxy"] --> server["目標伺服器"]
```
...
```mermaid
flowchart LR
    client["客戶端"] --> proxy["Reverse Proxy"] --> server["後端伺服器"]
```
...
```mermaid
flowchart LR
    browser["瀏覽器"] --> dev["localhost:4200/api"] --> proxy["Dev Server Proxy"] --> api["https://api.example.com"]
```
