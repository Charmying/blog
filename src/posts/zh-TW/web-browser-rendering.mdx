...
```mermaid
flowchart LR
    dom["DOM + CSSOM"] --> render["Render Tree"] --> layout["Layout"] --> paint["Paint"] --> compositing["Compositing"] --> screen["畫面"]
```
...
```mermaid
flowchart TD
    html["HTML 解析"] --> domtree["DOM Tree"]
    css["CSS 解析"] --> cssomtree["CSSOM Tree"]
...
瀏覽器渲染是前端應用完整執行週期的最後一段。不同渲染策略 (CSR / SSR / SSG) 決定了初始 HTML 的內容，參考 [CSR vs SSR vs SSG](/articles/web-csr-ssr-ssg)；想了解完整執行週期，參考 [從原始碼到畫面](/articles/web-sourcecode-to-screen)。
