...
`Object.prototype` sits at the top of nearly every chain, and its `[[Prototype]]` is `null`, which is where the chain ends.
...
Because writes only ever touch the instance, putting a *mutable* object or array on a prototype is dangerous, as every instance ends up sharing the same one:
