export const metadata = {
  ...
  date: '2026-06-30',
  excerpt: '介紹創建型設計模式中的工廠方法模式：物件的建立延遲到子類別，讓建立邏輯可以在不修改原有程式碼的情況下擴充。',
  tags: ['軟體設計', '設計模式', 'OOP'],
};
...
它把建立物件這件事定義成一個介面 (也就是工廠方法)，再交由子類別決定要實體化哪個具體類別。
...
Factory Method 的價值，在於它結合了繼承帶來的共用邏輯，當 Creator 除了工廠方法之外還有共用的業務邏輯、整個範本方法 (Template Method) 結構已經就位時，就是它派上用場的時機。
...
這麼做的好處是，父類別不必知道具體型別就能操作物件，而子類別則掌控具體的建立方式。新增型別時，只要新增一個子類別，不必去改一條越長越長的 if-else，也不必動到既有的類別。
