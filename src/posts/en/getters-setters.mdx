...
`finalPrice` doesn't have a backing field. Every read calculates it fresh from `price` and `discount`. If either changes, the result is automatically correct — no manual sync required.
