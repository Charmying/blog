...
```text
Request → Empty HTML + JS → Execute JS → Fetch API → Render
```
...
```text
Request → Server fetches data → Generates HTML → Full HTML → Displays
```
...
```text
Build time: Fetch data → Generate all HTML files
User request → CDN serves static HTML instantly
```
