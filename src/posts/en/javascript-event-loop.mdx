...
```mermaid
flowchart TD
    s1["1. first() pushed onto the stack"]
    s2["2. second() pushed onto the stack"]
    s3["3. console.log('Hello') runs and is removed"]
    s4["4. second() finishes, removed"]
    s5["5. first() finishes, removed"]
    s1 --> s2 --> s3 --> s4 --> s5
```
...
```mermaid
flowchart TD
    s1["Run all synchronous code\n(Call Stack)"]
    s2["Drain the Microtask Queue\n(Promise callbacks, etc.)"]
    s3["Pick one task from the Task Queue\nand push it onto the Call Stack"]
    s1 --> s2 --> s3 --> s1
```
...
- **Web APIs**: handle timers, network requests, and events in the background
