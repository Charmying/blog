...
This pipeline runs the same way regardless of how the HTML was generated (CSR, SSR, or SSG). The rendering strategy determines what the browser receives; the steps from there are identical.
...
Elements on their own compositor layer (achieved with `transform: translateZ(0)` or `will-change: transform`) can be processed entirely on the GPU, skipping Layout and Paint entirely, making them the most performant to animate.
...
These only affect the GPU compositing step, with no Layout or Paint:
...
Browser rendering is the final stage of a broader frontend pipeline. The rendering strategy (CSR, SSR, or SSG) determines what HTML the browser starts with. See [CSR vs. SSR vs. SSG](/articles/web-csr-ssr-ssg) for a comparison, or [From Source to Screen](/articles/web-source-to-screen) for the full lifecycle.
