...
```mermaid
flowchart TD
    s1["1. Global EC pushed onto the stack"]
    s2["2. first() called → its EC pushed"]
    s3["3. second() called → its EC pushed"]
    s4["4. second() finishes → EC popped"]
    s5["5. first() finishes → EC popped"]
    s6["6. Back to Global EC"]
    s1 --> s2 --> s3 --> s4 --> s5 --> s6
```
