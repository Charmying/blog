...
少了 GPU 協助處理圖層合成 (Compositing)，渲染工作回落至 CPU。這直接造成 Speed Index 與 LCP 的結構性下 —— 是頁面變差了，而是測量環境的硬體支援不如日常瀏覽器。
...
- V8 Major GC — 批量測試期間記憶體堆積，V8 會周期性觸發 Major GC (Mark-Sweep-Compact)。如果 GC 週期剛好與稽核視窗重疊，TBT 可能偏移 ±15 分，完全取決於時機點。
