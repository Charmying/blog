...
2. 第二階段 (延遲渲染)：純 CSR 頁面在第一階段只有空殼，Googlebot 會把它放進「渲染佇列」，等到雲端計算資源空閒再執行 JavaScript，這個延遲可能是數天到數週。
...
這條管線的快慢直接影響 Core Web Vital：FCP (First Contentful Paint) 衡量使用者首次看到有意義內容的時間；LCP (Largest Contentful Paint) 衡量最大可見元素完成渲染的時間。Chrome DevTools 的 Performance 面板可以錄製完整的渲染過程，幫助找出瓶頸所在。

### 效能關鍵：只觸發 Composite，跳過 Layout 和 Paint

修改 `top`、`left` 等幾何屬性會觸發 Layout → Paint → Composite 的完整流程，每一幀都重新計算，代價高。

`transform` 和 `opacity` 則只觸發 Compositing，直接交給 GPU 處理，完全繞過 Layout 和 Paint，動畫流暢度明顯更好：
