...
State encapsulates each of an object's states as a separate class. When the state changes, the object delegates to the new state object, swapping behavior automatically, with no if-else chains needed.
