...
export function ArticleList({ posts, tags, series }: ArticleListProps) {
  ...
  const activeSeries = urlSeries && series.includes(urlSeries) ? urlSeries : null;
  ...
  }, [posts, activeTag, activeSeries, sortBy]);
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {series.length > 0 && (
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              <Link href={activeTag ? `/articles?tag=${encodeURIComponent(activeTag)}` : "/articles"} className={`article-tag article-tag--series ${!activeSeries ? "article-tag--active" : ""}`}>
                {t("allSeries")}
              </Link>
              {series.map((s) => (
                <Link key={s} href={`/articles?series=${encodeURIComponent(s)}${activeTag ? `&tag=${encodeURIComponent(activeTag)}` : ""}`} className={`article-tag article-tag--series ${activeSeries === s ? "article-tag--active" : ""}`}>
                  {getSeriesName(s, locale)}
                </Link>
              ))}
            </div>
          </div>
        ...
        {tags.length > 0 && (
          <div className="mb-10 xs:mb-12 sm:mb-16">
            <div className="flex flex-wrap gap-2 mb-8">
              <Link href={activeSeries ? `/articles?series=${activeSeries}` : "/articles"} className={`article-tag ${!activeTag ? "article-tag--active" : ""}`}>
                {t("all")}
              </Link>
              {tags.map((tag) => (
                <Link key={tag} href={`/articles?${activeSeries ? `series=${encodeURIComponent(activeSeries)}&` : ""}tag=${encodeURIComponent(tag)}`} className={`article-tag ${activeTag === tag ? "article-tag--active" : ""}`}>
                  {tag}
                </Link>
              ))}
            </div>
                ...
                <div className="view-toggle" role="group" aria-label={t("viewToggleLabel")}>
                  ...
                  <button type="button" onClick={() => setViewMode("list")} aria-label={t("viewList")} aria-pressed={viewMode === "list"} className={`view-toggle__btn ${viewMode === "list" ? "view-toggle__btn--active" : ""}`}>
                    <svg className="w-4.5 h-4.5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" aria-hidden="true">
                      ...
                      <rect x="3" y="4.5" width="2" height="3" rx="0.5" />
                      <rect x="3" y="10.5" width="2" height="3" rx="0.5" />
                      <rect x="3" y="16.5" width="2" height="3" rx="0.5" />
                    </svg>
                  </button>
                </div>
        ...
        {sorted.length === 0 ? (
          ...
          <div className={viewMode === "grid" ? "article-grid" : "article-list"}>
            {sorted.map((post) =>
              viewMode === "grid" ? (
                <Link key={post.slug} href={`/articles/${post.slug}`} className="article-card">
                  {/* Series breadcrumb label */}
                  {post.series && (
                    <div className="article-card__series">
                      <span className="article-card__series-name">
                        {getSeriesName(post.series, locale)}
                      </span>
                      {post.category && (
                        <>
                          <span className="article-card__series-sep">›</span>
                          <span className="article-card__series-name">
                            {getSeriesName(post.category, locale)}
                          </span>
                        </>
                      )}
                    </div>
                ...
                <Link key={post.slug} href={`/articles/${post.slug}`} className="article-row">
                  ...
                  <div className="article-row__tags">
                    {post.tags.slice(0, 3).map((tag) => (
                      <span key={tag} className="article-row__tag">
                        {tag}
                      </span>
                    ))}
                  </div>
