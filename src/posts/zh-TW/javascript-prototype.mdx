...
每個物件內部都藏著一條連到另一個物件的連結。當你存取一個物件上不存在的屬性時，JavaScript 會沿著這條連結往上，這就是原型鏈 (prototype chain)。
...
正因為寫入只會動到實例自己，所以在原型上放「可變的物件或陣列」非常危險，因為會被所有實例共用同一份：
...
1. `Object.create(proto)`：用指定的原型建立新物件
2. 建構函式搭配 `new`
3. `class`：本質仍是原型 (下一節說明)
...
`extends` 則會同時接起「兩條」原型鏈：實例的鏈，以及靜態 (static) 成員的鏈：
