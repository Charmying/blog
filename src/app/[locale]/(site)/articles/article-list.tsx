        ...
        {series.length > 0 && (
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href={activeTag ? `/articles?tag=${encodeURIComponent(activeTag)}` : "/articles"}
                className={`article-tag article-tag--series ${!activeSeries ? "article-tag--active" : ""}`}
              >
                {t("allSeries")}
              </Link>
              {series.map((s) => (
                <Link
                  key={s}
                  href={`/articles?series=${encodeURIComponent(s)}${activeTag ? `&tag=${encodeURIComponent(activeTag)}` : ""}`}
                  className={`article-tag article-tag--series ${activeSeries === s ? "article-tag--active" : ""}`}
                >
        ...
        {tags.length > 0 && (
          <div className="mb-10 xs:mb-12 sm:mb-16">
            <div className="flex flex-wrap gap-2 mb-8">
              <Link
                href={activeSeries ? `/articles?series=${activeSeries}` : "/articles"}
                className={`article-tag ${!activeTag ? "article-tag--active" : ""}`}
              >
                {t("all")}
              </Link>
              {tags.map((tag) => (
                <Link
                  key={tag}
                  href={`/articles?${activeSeries ? `series=${encodeURIComponent(activeSeries)}&` : ""}tag=${encodeURIComponent(tag)}`}
                  className={`article-tag ${activeTag === tag ? "article-tag--active" : ""}`}
                >
                  {tag}
                </Link>
              ))}
